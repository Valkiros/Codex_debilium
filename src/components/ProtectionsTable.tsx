...
export const ProtectionsTable: React.FC<ProtectionsTableProps> = ({ items, onItemsChange, referenceOptions, defaultItem, bouclierActif }) => {
    ...
    const calculateTotalPr = (base: string | number, modif: string): string | number => {
        const baseVal = parseInt(base as string) || 0;
        const modifVal = parseInt(modif) || 0;
                        ...
                        {items.map((item) => {
                            ...
                            return (
                                <tr key={item.id} className={`border-b border-leather-light/30 hover:bg-leather/5 transition-opacity duration-300 ${isInactive ? 'opacity-50 grayscale' : ''}`}>
                                    ...
                                    {/* Pr Sol */}
                                    <td className="p-2 text-center font-bold">{calculateTotalPr(refPrSol, item.modif_pr_sol || '')}</td>
                                    <td className="p-2">
                                        <input
                                            type="text"
                                    ...
                                    {/* Pr Spe */}
                                    <td className="p-2 text-center font-bold">{calculateTotalPr(refPrSpe, item.modif_pr_spe || '')}</td>
                                    <td className="p-2">
                                        <input
                                            type="text"
                                    ...
                                    {/* Pr Mag */}
                                    <td className="p-2 text-center font-bold">{calculateTotalPr(refPrMag, item.modif_pr_mag || '')}</td>
                                    <td className="p-2">
                                        <input
                                            type="text"
