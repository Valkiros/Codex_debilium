...
interface CatalogueProps {
    ...
    isGlobalCondensed: boolean;
    onIsGlobalCondensedChange: (condensed: boolean) => void;
}

export const Catalogue: React.FC<CatalogueProps> = ({ items = [], onItemsChange, isGlobalCondensed = false, onIsGlobalCondensedChange }) => {
    ...
    return (
        <div className="h-full flex flex-col space-y-4 animate-fade-in relative">
            <div className="flex-none bg-parchment/50 p-4 rounded-lg border border-leather/10 shadow-sm relative pt-16">
                ...
                <div className="absolute top-0 left-0 right-0 bg-leather text-parchment p-3 rounded-t-lg shadow-md flex justify-between items-center z-10 w-full mb-4 px-6">
                    <div className="flex items-center gap-4">
                        ...
                        <label className="flex items-center gap-2 ml-4 cursor-pointer font-bold text-parchment/90 select-none text-sm bg-parchment/10 px-3 py-1.5 rounded border border-white/20 hover:bg-parchment/20 transition-colors">
                            <input
                                type="checkbox"
                                checked={isGlobalCondensed}
                                onChange={(e) => {
                                    const checked = e.target.checked;
                                    onIsGlobalCondensedChange(checked);
                                    onItemsChange(items.map(item => ({ ...item, is_condensed: checked })));
                                }}
                                className="w-4 h-4 rounded border-white/30 text-leather focus:ring-leather cursor-pointer"
                            />
                            <GiEyeTarget className="text-lg text-gold" /> Cacher les informations
                        </label>
                    </div>
            ...
            <div className="space-y-8 flex-1 overflow-y-auto pr-2 pb-4 min-h-0">
                                            ...
                                            return (
                                                <tr key={item.uid} className={`hover:bg-leather/5 transition-colors group ${isExcluded ? 'opacity-50 grayscale bg-gray-50' : ''}`}>
                                                    ...
                                                    <td className="p-2 text-right border-l border-leather/10 bg-parchment sticky z-30 shadow-[-2px_0_5px_rgba(0,0,0,0.05)]" style={{ right: 130, width: 100, minWidth: 100, maxWidth: 100 }}>
                                                        {item.is_condensed ? (
                                                            <span className="opacity-20 block w-full text-center">-</span>
                                                        ) : (
