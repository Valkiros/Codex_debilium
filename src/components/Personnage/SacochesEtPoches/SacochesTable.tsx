...
interface SacochesTableProps {
    items: Equipement[];
    onItemsChange: (items: Equipement[]) => void;
    referenceOptions: RefEquipement[];
}

export const SacochesTable: React.FC<SacochesTableProps> = ({ items, onItemsChange, referenceOptions }) => {
    ...
    const getRefValue = (refId: number, key: string, subKey?: string) => {
        ...
        if (!r) return '';
        if (subKey && r.raw && r.raw.details) return r.raw.details[subKey]; // Fallback to raw details
        if (subKey && (r as any)[key]) return (r as any)[key][subKey]; // Try nested
        return (r as any)[key] || ''; // Direct access
    };
    ...
    return (
        <div className="mb-6 p-4 bg-parchment/30 rounded border border-leather/20">
            <div className="flex justify-between items-center mb-2 border-b border-leather/20 pb-1">
                <h3 className="font-bold text-leather uppercase">Sacoches</h3>
                        ...
                        return (
                            <tr key={item.uid} className="border-b border-leather/10 hover:bg-leather/5">
                                ...
                                <td className="p-2 text-center text-ink-light">
                                    {refItem?.places || getRefValue(item.refId, 'details', 'places') || '-'}
                                </td>
