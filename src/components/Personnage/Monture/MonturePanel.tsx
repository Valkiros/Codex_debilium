...
const emptyMount: Omit<Mount, 'uid'> = {
    nom: 'Nouvelle Monture',
    pv_current: 20,
    pv_max: 20,
    courage: 0,
    intelligence: 0,
...
export const MonturePanel: React.FC<MonturePanelProps> = ({ mounts = [], onMountsChange }) => {
    ...
    const [mountToDelete, setMountToDelete] = useState<string | null>(null);

    const handleAddMount = () => {
        onMountsChange([...mounts, { ...emptyMount, uid: uuidv4() }]);
    };

    const handleRemoveMount = (uid: string) => {
        setMountToDelete(uid);
    };

    const confirmRemoveMount = () => {
        if (mountToDelete) {
            onMountsChange(mounts.filter(m => m.uid !== mountToDelete));
            setMountToDelete(null);
        }
    };

    const handleMountChange = (uid: string, field: keyof Mount, value: any) => {
        onMountsChange(mounts.map(m => m.uid === uid ? { ...m, [field]: value } : m));
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center bg-parchment/60 p-4 border border-leather/20 rounded shadow-sm">
                <div>
                    <h2 className="text-2xl font-serif font-bold text-leather capitalize">Montures</h2>
                    <p className="text-sm text-ink-light italic">Gérez les montures de votre personnage</p>
                </div>
                <button
            ...
            {mounts.length === 0 ? (
                <div className="text-center p-8 bg-parchment/30 border border-leather/20 rounded-lg text-ink-light font-serif italic">
                    Aucune monture pour l'instant. Cliquez sur "Ajouter" pour en créer une.
                </div>
            ) : (
                <div className="grid grid-cols-1 gap-6">
                    {mounts.map((mount) => (
                        <div key={mount.uid} className="bg-parchment/60 p-4 rounded-lg border border-leather/30 shadow-sm relative">
                            {/* Bouton de suppression */}
                            <button
                                onClick={() => handleRemoveMount(mount.uid)}
                                className="absolute top-4 right-4 text-red-700/60 hover:text-red-700 hover:bg-red-100/50 p-1.5 rounded transition-colors"
                                title="Supprimer cette monture"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <div className="space-y-4 lg:col-span-1">
                                    <div>
                                        ...
                                        <SmartInput
                                            type="text"
                                            value={mount.nom}
                                            onCommit={(val) => handleMountChange(mount.uid, 'nom', val)}
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-red-900/10 p-3 rounded-lg border border-red-900/20">
                                            ...
                                            <div className="flex items-center justify-center gap-2">
                                                <SmartInput
                                                    type="number"
                                                    value={mount.pv_current}
                                                    onCommit={(val) => handleMountChange(mount.uid, 'pv_current', val)}
                                                ...
                                                <SmartInput
                                                    type="number"
                                                    value={mount.pv_max}
                                                    onCommit={(val) => handleMountChange(mount.uid, 'pv_max', val)}
                                                    className="w-16 bg-white/50 border border-red-900/30 rounded p-1 text-center font-bold text-red-900 text-lg outline-none focus:border-red-900 focus:bg-white transition-colors"
                                                />
                                            </div>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <SmartInput
                                                type="text"
                                                value={mount.prix}
                                                onCommit={(val) => handleMountChange(mount.uid, 'prix', val)}
                                        ...
                                        <div>
                                            ...
                                            <SmartInput
                                                type="text"
                                                value={mount.entretien}
                                                onCommit={(val) => handleMountChange(mount.uid, 'entretien', val)}
                                                className="w-full bg-input-bg border border-leather/30 rounded px-3 py-2 text-leather-dark focus:border-leather outline-none text-sm"
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <SmartInput
                                                type="number"
                                                value={mount.mvt_marche}
                                                onCommit={(val) => handleMountChange(mount.uid, 'mvt_marche', val)}
                                        ...
                                        <div>
                                            ...
                                            <SmartInput
                                                type="number"
                                                value={mount.mvt_course}
                                                onCommit={(val) => handleMountChange(mount.uid, 'mvt_course', val)}
                                                className="w-full bg-input-bg border border-leather/30 rounded px-3 py-2 text-center text-leather-dark font-bold focus:border-leather outline-none"
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <SmartInput
                                            type="number"
                                            value={mount.mvt_voyage}
                                            onCommit={(val) => handleMountChange(mount.uid, 'mvt_voyage', val)}
                                            className="w-full bg-input-bg border border-leather/30 rounded px-3 py-2 text-center text-leather-dark font-bold focus:border-leather outline-none"
                                        />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <SmartInput
                                            type="text"
                                            value={mount.lieux}
                                            onCommit={(val) => handleMountChange(mount.uid, 'lieux', val)}
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <SmartInput
                                                type="text"
                                                value={mount.charge_max}
                                                onCommit={(val) => handleMountChange(mount.uid, 'charge_max', val)}
                                                className="w-full bg-input-bg border border-leather/30 rounded px-3 py-2 text-leather-dark focus:border-leather outline-none text-sm"
                                        ...
                                        <div>
                                            ...
                                            <SmartInput
                                                type="text"
                                                value={mount.bonus_cavalier}
                                                onCommit={(val) => handleMountChange(mount.uid, 'bonus_cavalier', val)}
                                                className="w-full bg-input-bg border border-leather/30 rounded px-3 py-2 text-leather-dark focus:border-leather outline-none text-sm"
                                                placeholder="Ex: +1 AT"
                                            />
                                        </div>
                                        ...
                                        <div className="grid grid-cols-5 md:grid-cols-10 gap-2">
                                            {[
                                                ...
                                                <div key={stat.key} className="flex flex-col items-center">
                                                    ...
                                                    <SmartInput
                                                        type="number"
                                                        value={mount[stat.key as keyof Mount]}
                                                        onCommit={(val) => handleMountChange(mount.uid, stat.key as keyof Mount, val)}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div>
                                            ...
                                            <SmartInput
                                                ...
                                                onCommit={(val) => handleMountChange(mount.uid, 'description', val)}
                                        ...
                                        <div>
                                            ...
                                            <SmartInput
                                                type="textarea"
                                                value={mount.competences}
                                                onCommit={(val) => handleMountChange(mount.uid, 'competences', val)}
                                                className="w-full h-32 bg-input-bg border border-leather/30 rounded p-3 text-leather-dark text-sm focus:border-leather outline-none resize-none"
                                        ...
                                        <div>
                                            ...
                                            <SmartInput
                                                type="textarea"
                                                value={mount.at_speciales}
                                                onCommit={(val) => handleMountChange(mount.uid, 'at_speciales', val)}
                                                className="w-full h-32 bg-input-bg border border-leather/30 rounded p-3 text-leather-dark text-sm focus:border-leather outline-none resize-none"
            ...
            {mountToDelete && (
                <div className="fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
                    <div className="bg-parchment rounded-xl shadow-2xl p-6 max-w-sm w-full border-2 border-leather/30 text-center space-y-4">
                        ...
                        <h3 className="text-xl font-bold font-serif text-leather-dark">Supprimer la monture</h3>
                        <p className="text-ink-light text-sm">
                            Êtes-vous sûr de vouloir supprimer cette monture ? Cette action est irréversible.
                        </p>
                        <div className="flex justify-center gap-4 pt-4">
                            <button
                                onClick={() => setMountToDelete(null)}
